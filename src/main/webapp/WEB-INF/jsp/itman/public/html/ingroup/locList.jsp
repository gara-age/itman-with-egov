...
<body>
	...
	<div id="contents">
        ...
        <?=$test?>
		<!-- 글쓰기 버튼-->
		<p class="addContent"><a onclick="window.open('../popup/contWriteItmLocation.php?idx=<?= $row['LOC_IDX']; ?>', 'EditPopUp', 'width=500, height=500, status=no,toolbar=no,scrollbars=no')" class="edit">><span></span><span></span><span></span></a></p>
		<div class="Basic">
			<ul class="adminList">
                <li class="tit">
					<p class="num">코드번호</p>
                ...
                <?php while ($row= (mysqli_fetch_array($query))) { ?>
                    <li>
                        <p class="num"><?= $row['LOC_CODE']; ?></p>
                        <p class="tit"><?= $row['LOC_NAME']; ?></p>
                        <p class="editDel">
							<a onclick="window.open('../popup/contEditItmLocation.php?idx=<?= $row['LOC_IDX']; ?>', 'EditPopUp', 'width=500, height=500, status=no,toolbar=no,scrollbars=no')" class="edit">수정</a><a onclick="window.open('../popup/listDelete.php?id=loc_del&target=<?= $row['LOC_IDX'] ?>', 'EditPopUp', 'width=500, height=500, status=no,toolbar=no,scrollbars=no')" class="del">삭제</a>
						</p>
                    </li>
					<?php } if($count == 0) { ?> 
